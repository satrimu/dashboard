...
class LogController extends Controller
{
    public function __construct(
        private readonly SecurityLogService $logService
    ) {}
     ...
     */
    public function archiveShow(Request $request, string $archiveFilename): InertiaResponse
    {
     ...
     */
    public function downloadArchive(string $archiveFilename): BinaryFileResponse
    {
        // Validate filename to prevent path traversal
        if (! preg_match('/^security-logs-\d{4}-\d{2}\.(zip|gz)$/', $archiveFilename)) {
            abort(404);
     ...
     */
    public function archiveNow(Request $request): JsonResponse
    {
        // Check if user can view activities (admin only)
